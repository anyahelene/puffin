...
LOGGING = {
    "version": 1,
    "formatters": {
        "app": {"format": "%(levelname)-7s: .%(module)s – %(message)s"},
        "simple": {"format": "%(levelname)-7s: %(name)s – %(message)s"},
        "app_request": {
            "()": RequestFormatter,
    ...
    def static(path: str, relativize=False):
        if relativize:
            return posixpath.relpath(
                static_url_for(path), start=posixpath.dirname(request.path) + "/"
            )
        else:
...
def shell_setup():
    ...
    def reload_all():
        ...
        for m in list(sys.modules.values()):
            if (
                type(m) == type(puffin)
                and m.__name__.startswith("puffin")
                and m.__name__ != __name__
                and not m.__name__.startswith("puffin.db")
            ):
                print("Reloading", m.__name__)
                reload(m)

    __scope__["reload_all"] = reload_all
    __scope__["__scope__"] = __scope__
    return __scope__
